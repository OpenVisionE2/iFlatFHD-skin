...
config.plugins.iFlatFHD.refreshInterval = ConfigNumber(default = "10")
config.plugins.iFlatFHD.woeid = ConfigNumber(default = "56438032")
config.plugins.iFlatFHD.tempUnit = ConfigSelection(default = "Celsius", choices = [("Celsius", _("Celsius")),("Fahrenheit", _("Fahrenheit"))])
...
class VWeather(Converter, object):
	...
	def getCF(self):
		if config.plugins.iFlatFHD.tempUnit.value == "Fahrenheit":
			return "°F"
		else: 
			return "°C"
...
class WeatherData:
	def __init__(self):
		...
		if config.plugins.iFlatFHD.refreshInterval.value > 0:
			self.timer = eTimer()
	...
	def GetWeather(self):
		timeout = config.plugins.iFlatFHD.refreshInterval.value * 1000 * 60
		if timeout > 0:
			self.timer.start(timeout, True)
			print "iFlatFHD lookup for ID " + str(config.plugins.iFlatFHD.woeid.value)
			url = "http://query.yahooapis.com/v1/public/yql?q=select%20item%20from%20weather.forecast%20where%20woeid%3D%22"+str(config.plugins.iFlatFHD.woeid.value)+"%22&format=xml"
